...
### E-CC5-014-S1 — targeted monitoring-boundary deployment

- Date: 2026-09-04 Asia/Manila. Same snapshot-backed staging VM 127; exact owner implementation `41c521703579ad0bff65d5fb5cd1773fbfdc67f1`. Five production sources were installed at six existing targets, including both dialer class copies. The full installer was not rerun.
- Local evidence: executable RED/GREEN covered retirement of the direct endpoint, authenticated campaign type/ID validation, fixed bound SQL, AMI/ECCP queue-status propagation and legacy-compatible browser calls. Local checks passed all eight PHP campaign regressions, the JavaScript boundary test, relevant syntax checks and installer regressions; no forbidden request-to-process boundary remained in the changed paths. The eight campaign tests and JavaScript boundary test were refreshed on September 4.
- First attempt: the single web assertion observed empty HTTP `200`, so deployment failed and restored all six original files. Separate independent read-only verification confirmed original hashes/modes, schema/grants, zero-use state and healthy services. This attempt is failed evidence, not a successful install.
- Separately approved second attempt: a newly reviewed wrapper preserved the failed attempt, used fresh evidence/source directories, and added bounded exact-response verification plus rollback-web convergence. Eleven local behavior cases passed, including delayed publication, timeout, malformed/trailing-newline responses, transport failure and rollback. These tests use controlled HTTP/clock boundaries, not a reproduction of remote PHP caching.
- Execution: the second wrapper ran once and exited `0`. All six installed target hashes matched; five PHP fixture tests passed against source and again against copies of installed PHP files. JavaScript was hash-verified, not executed on staging. The first parameter-free web probe, approximately 6.5 seconds after activation, returned HTTP `410`, 67 bytes, exact JSON `{"status":"error","message":"This endpoint is no longer available"}`. The first attempt's cache-timing explanation remains unproven.
- Stable state: schema SHA-256 `56a9513d0ad74f71a02360b16ba2d9286540eb6560caf390322595e4a21f85e6`; seven-table grants SHA-256 `8e2992584e55e8d445e939f06faca94c894b7b586f29e15c78ccc8a8274789fa`; routes, active campaigns/current calls and channels `0`; dialer active/enabled as `asterisk`; Asterisk active at `18.19.0`; HTTPS `200`.
- Protected evidence: `/root/callcenter-phase1/cc5014-41c5217-r2`, directory mode `700`, result/log/body mode `600`, backups retain original modes. Result SHA-256 `2ef42e5737f8ee488b0e13e46d13b48b0313f3d4775601345a2c5222ea9e50d3`; log `98fa56c7215196ae4daca1d154d803bd8083f3d485d003b588a7f0b8c49b2419`; endpoint body `ae3718c91040e2bb337b4bb5b1097a260e2e64ea9933b2f87124921555cf7b5e`. First-attempt evidence remains intact.
- Separate root-side read-only postcheck exited `0`. A fresh independent verifier's read-only audits also exited `0` and confirmed host identity, clean exact source, all six installed/source/backup hashes and modes, fixture hashes and protected PASS records, exact live HTTP `410` JSON, stable schema/grants, zero-use state, service health, evidence hashes/modes and preserved first-attempt evidence. No temporary deployment files, probe directories or pending results remained. The verifier did not rerun fixtures or the deployment.
- Scope: local fixtures, native installed-source behavior and the parameter-free retired endpoint only. No authenticated browser workflow, live campaign/queue request, unavailable-agent AMI transition, call, route/database change, PHP-FPM restart or cache configuration change was exercised. HTTPS used loopback with certificate validation disabled.
...
- No authenticated browser UI, agent, inbound/outbound call-flow, recording, retry/callback, or report workflow has passed. CC5-010 has a rollback-isolated class/database staging cycle; CC5-011 through CC5-014 have installed-source/request-bootstrap and bounded parameter-free evidence only; no external call was placed.
...
- CC5-013 sent no campaign or queue parameter. Its separate request-to-shell finding was implemented in CC5-014; local fixtures and the targeted retired-endpoint response do not establish an authenticated monitoring UI or live AMI/call-flow result. No malicious live input was executed.
